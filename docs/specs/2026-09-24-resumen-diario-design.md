...
- `/detalle <nombre>`: el detalle de una persona (partidas, juegos, oros, días jugados, juego más jugado y, por juego, partidas, oros, mejor puntaje y racha actual/récord), más una sección "Hoy" con los juegos que ya jugó y los que le faltan. Sin nombre muestra el propio; respondiendo a un mensaje, el de quien lo mandó. El nombre exacto (sin importar mayúsculas ni tildes) gana sobre el comienzo; si el comienzo coincide con varios, los lista; si dos personas se llaman exactamente igual, pide responder a un mensaje.
